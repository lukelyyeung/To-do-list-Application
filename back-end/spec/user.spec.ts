...
describe("The user functions should be able to", function () {
    ...
    it("will throw error when fail to read user file.", function (done) {
        eraseFile()
            ...
            .catch(err => expect(() => { throw err }).toThrow(new Error(USER.LOAD_USER_FILE_FAIL)))
            .then(() => done());
    })
    ...
    it("will throw error when fail to write file during add user.", function (done) {
        eraseFile()
            ...
            .then(() => { throw new Error('Promise should not be resolved') })
            .catch(err => expect(() => { throw err }).toThrow(new Error(USER.WRITE_USER_FILE_FAIL)))
            .then(() => done());
    })
    ...
    it("will throw error when fail to write file during add user.", function (done) {
        eraseFile()
            ...
            .then(() => { throw new Error('Promise should not be resolved') })
            .catch(err => expect(() => { throw err }).toThrow(new Error(USER.UPDATE_USER_FILE_FAIL)))
            .then(() => done());
    })
})